...
def main(args):
    ...
    name = (
        ...
        f"_iter{args.iter}_warmup{args.warmup}_target{target}_inner_loop{args.innerLoop}"
        f"_geometry{args.geometry}_patch_size{args.patch_size}_seed42-{exp_id}"
    )
    ...
    path = f"{pwd}/run/straight_attack/{exp_id}"
    os.makedirs(path, exist_ok=True)

    import sys

    sys.path.append(f"{pwd}/VLAAttacker/white_patch")
    from straight_attack_ddp import OpenVLAAttacker
...
if __name__ == "__main__":
    ...
    parser.add_argument("--targetAction", type=list_of_floats, default=[1.0])
